...
const CartItem = ({order, handleRemoveOrders,handleOrderConfirm}) => {
                ...
                <tbody className="">
                    ...
                    <tr className="flex items-center justify-between">
                        <th className="w-12 md:w-fit">
                            <button onClick={() => handleRemoveOrders(_id)} className="text-lg btn btn-error btn-sm btn-circle">X</button>
                        </th>
                        ...
                        <td className="w-22 md:w-fit p-2"> <span>{date}</span> <br />
                        ...
                        <th>
                            {
                                ...
                                <div onClick={(event)=> handleOrderConfirm(event, _id)} className="flex flex-col gap-2">
                                    <button disabled={status === 'pending' ? false : true} className="btn btn-xs btn-primary">Confirm</button> 
                                    <button disabled={status === 'pending' ? false : true} className="btn btn-xs btn-error">Cancel</button> 
                                </div>
                                :
