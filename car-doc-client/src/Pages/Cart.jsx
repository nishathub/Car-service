...
const Cart = () => {
  ...
  // const URL = isAdmin ? `https://car-doc-server-5h2za3pf1-nishats-projects-890e0902.vercel.app/allOrders` : `https://car-doc-server-5h2za3pf1-nishats-projects-890e0902.vercel.app/allOrders?email=${user?.email}`;
  const URL = `https://car-doc-server-5h2za3pf1-nishats-projects-890e0902.vercel.app/allOrders?email=${user?.email}`;
  useEffect(() => {
    if (!isLoading) {
      // this conditioning is very crucial to avoid unnecessary fetching empty data
      fetch(URL, {
        credentials: "include",
      })
        .then((res) => res.json())
  ...
  const handleRemoveOrders = (id) => {
    console.log(id);
    fetch(
      `https://car-doc-server-5h2za3pf1-nishats-projects-890e0902.vercel.app/allOrders/${id}`,
      {
        method: "DELETE",
      }
    )
          ...
          <div>
            {!user ? (
              <p className="text-3xl text-center">
                Log in to see items in the cart
              </p>
            ) : (
              <div>
                {orders.length === 0 ? (
                  <div className="text-center h-screen">
                    ...
                    <p className="mt-4 text-xl">
                      Add some services to keep your vehicle smooth
                    </p>
                  </div>
                ) : (
                  <div>
                    <div>
                      {isAdmin && (
                        <div>
                          <p className="text-yellow-400 font-bold text-right my-4">
                            You are Admin
                          </p>
                        </div>
                      )}
